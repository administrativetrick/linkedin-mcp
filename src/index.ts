...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (name === "search_linkedin_jobs") {
    try {
      ...
      const jobs = await linkedIn.query(linkedInQuery);
      ...
      jobs.forEach((job: any, index: number) => {
        responseText += `${index + 1}. ${job.position}\n`;
        responseText += `   Company: ${job.company}\n`;
        responseText += `   Location: ${job.location}\n`;
        responseText += `   Posted: ${job.agoTime || job.date}\n`;
        if (job.salary) {
          responseText += `   Salary: ${job.salary}\n`;
        }
        responseText += `   Apply: ${job.jobUrl}\n\n`;
      });
